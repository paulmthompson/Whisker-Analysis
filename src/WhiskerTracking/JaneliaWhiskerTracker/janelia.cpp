...
void JaneliaTracker::compute_seed_from_point_field_on_grid(const Image<uint8_t> &image, Image<uint8_t> &hist,
                                                           Image<float> &slopes, Image<float> &stats) {
    ...
    // Vertical lines
    {
        ...
        for (x = 0; x < stride; x += this->config._lattice_spacing) {
            for (y = 0; y < image.height; y++) {
                ...
                {
                    p = newp;
                    std::optional<Seed> s = compute_seed_from_point_ex(image, x + y * stride, this->config._maxr, &m,
                                                                       &stat);
                    if (!s) break;
                    newp = s->xpnt + stride * s->ypnt;
                    if (newp == p || stat < this->config._iteration_thres)
        ...
        if (issa) {
            for (i = 0; i < support; i++) {
                ...
                for (j = 0; j < support; j++) {
                    ...
                    {
                        this->pxlist[ioob--] = std::move(
                                offset_pair(ww * std::min(std::max(0, ty), hh - 1) + std::min(std::max(0, tx), ww - 1),
        ...
        while (line.score > sigmin) {
            ...
            trusted = adjust_line_start(&line, image, &p, &roff, &rang, &rwid);
            {
                ...
                while (!trusted /*&& score > sigmin*/ && nmoves < this->config._half_space_tunneling_max_moves) {
                    ...
                    trusted &= adjust_line_start(&line, image, &p, &roff, &rang, &rwid);
                    if (trusted && line.score < sigmin) { // check to see if a line can be reaquired
        ...
        while (line.score > sigmin) {
...

            {
                ...
                while (!trusted /*&& score > sigmin*/ && nmoves < this->config._half_space_tunneling_max_moves) {
                    ...
                    trusted &= adjust_line_start(&line, image, &p, &roff, &rang, &rwid);
                    if (trusted && line.score < sigmin) { // check to see if a line can be reaquired
                        std::optional<Seed> sd = compute_seed_from_point(image, p,
                                                                         3.0); // this will often pop the line back on
...
float JaneliaTracker::eval_half_space(Line_Params *line, const Image<uint8_t> &image, int p, float *rr, float *ll) {
    ...
    while (i < npxlist) {
        ...
        r += parray[this->pxlist[i].image_ind] * this->half_space_bank.bank.data[righthalf +
                                                                                 this->pxlist[i].weight_ind]; // It doesn't work if I have the a term here. Why is that?
        i++;
...
int JaneliaTracker::adjust_line_start(Line_Params *line, const Image<uint8_t> &image, int *pp,
    ...
    if (is_change_too_big(backup, *line, this->config._max_delta_angle, this->config._max_delta_width,
                          this->config._max_delta_offset)) {
...
bool
JaneliaTracker::is_change_too_big(const Line_Params new_line, const Line_Params old, const float alim, const float wlim,
                                  const float olim) {
